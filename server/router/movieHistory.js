...
router.get("/3months", (req, res) => {
  ...
  var selectData = [reqQuery.emp_id, reqQuery.mem_name]
  db.query(
    `SELECT movie_id, movie name from movie NATURAL JOIN schedule NATURAL JOIN reservation where emp_id = ? OR emp_name & screen_beg = DATE_ADD(CURDATE, INTERVAL '-3'MONTH);`,
    selectData,
    function (err, rows) {
      if (err) throw err
      else res.send(rows)
